const Storage = {
  ...
  // Resume text (parsed)
  async getResumeText() {
    const useTailored = await this.get('useTailoredResume');
    const active = await this.get('activeResumeText');
    if (useTailored !== false && active) return active;
  ...
  // Gemini API key or Clyde Pro Token fallback
  async getApiKey() {
    const key = await this.get('geminiApiKey');
    if (key) return key;
    const proToken = await this.get('clydeProToken');
    if (proToken) return 'pro-placeholder';
    return null;
  },
