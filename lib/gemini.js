    ...
    for (let attempt = 1; attempt <= 4; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          if (transientStatuses.includes(response.status) && attempt < 4) {
          ...
          if (response.status === 429) {
            throw new Error(`Exceeded Gemini API rate limit. If on the free tier, wait a minute and try again. (HTTP 429)`);
          }
          if (response.status === 404) {
            throw new Error(`The requested model was not found. (HTTP 404)`);
          }
          if (response.status === 503 || response.status === 504) {
            throw new Error(`The AI service is temporarily overloaded or gateway timed out. Please try again in a moment. (HTTP ${response.status})`);
          }
