/* Workable Portal Handler for Clyde Apply
 */
...
PortalHandlers.register({
  ...
  detect(url) {
    return /workable\.com/i.test(url) || 
      !!document.querySelector('[data-testid*="workable"]') ||
  ...
  getFields() {
    ...
    const radioGroups = {};
    radios.forEach(r => {
      const name = r.name || 'workable_radio_group';
      if (!radioGroups[name]) {
        radioGroups[name] = [];
      }
  ...
  getJobInfo() {
    const title = document.querySelector('[data-testid="job-title"], h1, .job-title')?.textContent?.trim() || '';
    const company = document.querySelector('[data-testid="company-name"], .company-name, .logo img')?.alt?.trim() || '';
